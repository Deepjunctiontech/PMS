package in.junctiontech.pms;

/**
 * Created by lenovo on 14-Oct-15.
 */
interface PMSOtherConstant {
    int REPEAT_DURATION = 1000 * 15*60;
    int SPLASH_SCREEN_DURATION = 5000;
    int GPS_DURATION = 30000;
    String URL_UPDATE = "http://junctiondev.cloudapp.net/junctionERP/remoteapi/locationUpdate";
    String SENDING_NUMBER = "8889552622";
  //  String IMAGE_LOCATION_ORIGINAL = "5554";
  //  String IMAGE_LOCATION_THUMBNAIL = "5554";
}
